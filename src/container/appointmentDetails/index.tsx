...
class Appointmentdetails extends React.Component<IProps, IState> {
  RBSheet: any;
  ...
  componentDidMount() {
    ...
    const {getAppointmentDetail} = this.props;
    if (bookingId) {
  ...
  listAvailabelList = () => {
    ...
    let theme = appTheme.theme;
    const {appointmentDetail = []} = orderList;
    const {personnel = [], details = [], order = []} = appointmentDetail;
    const {basic = []} = personnel;
    const {
      name_first = '',
      name_last = '',
    ...
    //   order &&
    //   order.basic &&
    //   order.basic.checkout_at &&
    //   order.basic.checkout_at.time;
    ...
    // console.log('appointmentDetail', JSON.stringify(start));
    return (
      // <FlatList
      //   data={[1, 2, 3]}
      //   renderItem={stylistList => {
            ...
            <View>
              ...
              <View style={styles.flexRow}>
                ...
                <Text
                  ...
                  {/* {`${duration_minutes} mins at ${hour + minute + ` ` + AmOrPm} `} */}
                </Text>
              </View>
              <Text
                style={[
  ...
  render() {
    ...
    let price = order && order.basic && order.basic.price;
    let paymentState = order && order.basic && order.basic.payment_state;
